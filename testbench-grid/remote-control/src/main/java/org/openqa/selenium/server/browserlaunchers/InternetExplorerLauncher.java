...
import com.vaadin.testbench.util.CleanIE;
...
public class InternetExplorerLauncher implements BrowserLauncher {
    ...
    public InternetExplorerLauncher(BrowserConfigurationOptions browserOptions,
        ...
        if (System.getProperty("ensureCleanSession") != null) {
        ...
        // You can't just individually configure a browser for PI mode; it's a
        // server-level configuration parameter
        ...
        // if user didn't request PI, but the server is configured that way,
        // just switch up to PI
        proxyInjectionMode = globalProxyInjectionMode;
        if (proxyInjectionMode) {
            realLauncher = new ProxyInjectionInternetExplorerCustomProxyLauncher(browserOptions, configuration,
                    sessionId, browserLaunchLocation);
            return;
        }

        // the mode isn't "chrome" or "proxyInjection"; at this point it had
        // better be "proxy"
        if (!"proxy".equals(mode)) {
            throw new RuntimeException("Unrecognized browser mode: " + mode);
        }

        realLauncher = new InternetExplorerCustomProxyLauncher(browserOptions, configuration, sessionId,
