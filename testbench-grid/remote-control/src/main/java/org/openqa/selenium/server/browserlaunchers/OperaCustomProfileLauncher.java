...
public class OperaCustomProfileLauncher extends AbstractBrowserLauncher {
    ...
    // Opera has been a real pain for me (Lightbody), and so I'm adding a simple
    // hook in the browser launcher that lets
    // me define, in an amazingly ghetto way, define additional browser settings
    // to write out to the ini file. For
    // example, I will use this to pre-determine the window size and location
    // simply because it appears Opera 9.2
    // has a regression in which window.resizeTo() and window.moveTo() simply do
    // not work!
    ...
    public OperaCustomProfileLauncher(
        ...
        commandPath = browserLaunchLocation == null ? findBrowserLaunchLocation()
        ...
        if (!WindowsUtils.thisIsWindows()) {
            ...
            if (operaBin != null) {
                ...
                String libPath = WindowsUtils.loadEnvironment().getProperty(
                        libPathKey);
                exe
    ...
    protected void launch(String url) {
        try {
            ...
            if (WindowsUtils.thisIsWindows()) {
                cmdarray = new String[] { commandPath, "/settings",
                        opera6ini.getAbsolutePath(), url };
            } else {
                cmdarray = new String[] { commandPath, "-nosession",
                        "-personaldir",
    ...
    private File makeCustomProfile() throws IOException {
        ...
        out.println("Automatic Proxy Configuration URL="
                + proxyPAC.getAbsolutePath());
    ...
    public void close() {
        ...
        if (exitValue == 0) {
            log
                    .warn("Opera seems to have ended on its own (did we kill the real browser???)");
        }
        try {
    ...
    /**
     * Opera knows it's running by using a "parent.lock" file in the profile
     * directory. Wait for this file to go away (and stay gone)
     * 
     * @param timeout
     *            max time to wait for the file to go away
     * @param timeToWait
     *            minimum time to wait to make sure the file is gone
     ...
     * stops again; we need to wait a bit to make sure the file lock is really
     * gone.
     * 
     * @param lock
     *            the parent.lock file in the profile directory
     * @param timeToWait
     *            minimum time to wait to see if the file shows back up again.
     *            This is not a timeout; we will always wait this amount of time
     *            or more.
     * @return true if the file stayed gone for the entire timeToWait; false if
     *         the file exists (or came back)
     */
    ...
    public static void main(String[] args) throws Exception {
        OperaCustomProfileLauncher l = new OperaCustomProfileLauncher(
                new BrowserConfigurationOptions(),
                new RemoteControlConfiguration(), "CUSTFF", null);
        l.launch("http://www.google.com");
        int seconds = 15;
        System.out.println("Killing browser in " + Integer.toString(seconds)
                + " seconds");
