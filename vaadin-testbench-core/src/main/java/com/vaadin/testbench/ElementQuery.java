...
import static com.vaadin.testbench.TestBench.wrap;
...
public class ElementQuery<T extends TestBenchElement> {
     ...
     *
     * @param elementClass the type of element to look for and return
     */
     ...
     * Instantiate a new ElementQuery to look for the given type of element.
     *
     * @param elementClass the type of element to return
     ...
     *
     * @param searchContext a {@link SearchContext}; either a {@link TestBenchElement} or
     ...
     *
     * @return The element of the type specified in the constructor
     * @throws TimeoutException if no element is found after 10 seconds has elapsed
     */
    public T waitForFirst() {
        Object result = new WebDriverWait(getDriver(), 10)
                .until(driver -> {
                    try {
                        return first();
     ...
     * @param context        the element to start the search from or <code>null</code> for
