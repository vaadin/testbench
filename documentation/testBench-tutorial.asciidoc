...
*_NOTE!_* Mozilla Firefox changed important APIs in version 35. This means that TestBench versions 4.0.2 or older do not work with the latest versions of Firefox. It is recommended you use Firefox 34 or link:https://www.mozilla.org/en-US/firefox/organizations/all/[Firefox ESR] (currently at 31.x) to execute tests on Firefox until this can be fixed. A new release of TestBench will be made as soon as this is fixed.
