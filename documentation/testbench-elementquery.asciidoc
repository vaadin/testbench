...
The `$` method creates an [classname]#ElementQuery# that looks for the given element class. The method is available both for `TestBenchTestCase` (searches the whole current page) and in `TestBenchEleemnt` (searches inside the given element).
...
classes extend [classname]#TestBenchElement#, which in turn implements the
[interfacename]#WebElement# interface, so Selenium element API can also be used directly.
