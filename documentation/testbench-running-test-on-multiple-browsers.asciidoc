...
2. Add `ci-sauce` Maven dependency to your pom.xml
3. Configure pom.xml to have `sauce-connect-plugin` open a tunnel with a tunnel identifier to Sauce Labs test grid
4. Write your tests as described in <<testbench.grid.test-remote>>
...
The dependency for `ci-sauce` is
[source,xml]
----
<dependency>
...
You can define the tested browsers and their versions in an environment variable `TESTBENCH_BROWSERS` with a comma separated list e.g. `TESTBENCH_BROWSERS=chrome,firefox,safari-11,safari-9`.
