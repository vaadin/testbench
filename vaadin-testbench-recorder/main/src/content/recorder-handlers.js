...
Recorder.addEventHandler('clickLocator', 'click', function (event) {
    ...
    if (event.button == 0 && noSelection) {
        ...
        // VFilterSelect sends requests with a 200ms delay, which will cause waitForVaadin to fail
        // so we need to pause here after the mouseClick.
        var isFilterSelect = (new RegExp("v-filterselect-next")).test(clickable.className) || (new RegExp("v-filterselect-prev")).test(clickable.className);
        if (isFilterSelect) {
            this.record('pause', '200', '');
        }
