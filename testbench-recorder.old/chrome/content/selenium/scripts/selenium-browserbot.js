...
BrowserBot.prototype.triggerMouseEvent = function(element, eventType, canBubble, clientX, clientY, button) {
    ...
    if (element.fireEvent && element.ownerDocument && element.ownerDocument.createEventObject && document.createEventObject) { //IE
        if (!button && !screenX && !screenY && !clientX && !clientY && !this.controlKeyDown && !this.altKeyDown && !this.shiftKeyDown && !this.metaKeyDown) {
            ...
            try {
                window.event = evt;
           		window.event.shiftKey = this.shiftKeyDown;
            	window.event.ctrlKey = this.controlKeyDown;
            	window.event.altKey = this.altKeyDown;
            }
            catch(e) {
...
BrowserBot.prototype.modifyWindowToRecordPopUpDialogs = function(windowToModify, browserBot) {
    ...
    if (browserVersion.isHTA) {
    	...
    	if (typeof XPCNativeWrapper != "undefined" && "unwrap" in XPCNativeWrapper) {
            // Firefox4 won't call the newOpen method unless we unwrap the window (#6676)
...
BrowserBot.prototype.locateElementByVaadin = function(tkString, inDocument) {
        ...
        if (!c) {
            try {
                ...
                if (de.message == "Permission denied") {
		    		...
		    		// IE9 Will fail here and be unloading the page on refresh. So just retry as a real close will be caught soon.
				    if(this.pageUnloading == true){
						return false;
				    }
                    return true;
                }
