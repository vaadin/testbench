...
public class CrudExampleIT extends AbstractIT {
    ...
    private void createNewItemInGrid() {
        ...
        $(TextFieldElement.class).waitForFirst();
        $(TextFieldElement.class).withCaption("First Name").single()
                .setValue(TEST_FIRST_NAME);
        $(TextFieldElement.class).withCaption("Last Name").single()
                .setValue(TEST_LAST_NAME);
        $(TextFieldElement.class).withCaption("Email").single()
                .setValue(TEST_EMAIL);
        $(TextFieldElement.class).withCaption("Phone").single()
                .setValue(TEST_PHONE);
        $(DatePickerElement.class).withCaption("Date Of Birth").single()
                .setDate(TEST_BIRTHDAY);
        $(TextFieldElement.class).withCaption("Occupation").single()
                .setValue(TEST_OCCUPATION);
        $(TextFieldElement.class).withCaption("Role").single()
                .setValue(TEST_ROLE);
    ...
    private void selectLatestItemInGrid(GridElement grid) {
        ...
        for (int i = 0; i < rowCount; i++) {
            ...
            if (TEST_FIRST_NAME.equals(firstName)
                    && TEST_LAST_NAME.equals(lastName)) {
    ...
    private void deleteLatestItemInGrid() {
        ...
        for (int i = 0; i < rowCount; i++) {
            ...
            Assertions.assertFalse(
                    TEST_FIRST_NAME.equals(firstName)
                            && TEST_LAST_NAME.equals(lastName),
