...
/**
 * Manages process execution for the plugin.
 * Now uses Java implementations for HAR filtering, k6 conversion, and proxy recording.
 * Only k6 execution still requires an external process.
 */
public class NodeRunner {

    private static final Logger log = Logger.getLogger(NodeRunner.class.getName());
     ...
     *
     * @param proxyPort the port for the proxy to listen on
     * @param harFile   the output HAR file path
     * @throws MojoExecutionException if starting the proxy fails
     */
    public void startProxyRecorder(int proxyPort, Path harFile) throws MojoExecutionException {
        ...
        try {
            ...
            throw new MojoExecutionException("Failed to start proxy recorder", e);
        }
    }
     ...
     *
     * @param harFile the HAR file to filter
     * @throws MojoExecutionException if filtering fails
     */
    ...
    /**
     * Refactors a k6 test for Vaadin compatibility with default think time settings.
     *
     * @param inputFile  the input k6 test file
     * @param outputFile the output refactored test file
     * @throws MojoExecutionException if refactoring fails
     */
    public void refactorK6Test(Path inputFile, Path outputFile) throws MojoExecutionException {
        refactorK6Test(inputFile, outputFile, K6TestRefactorer.ThinkTimeConfig.DEFAULT);
    }

    /**
     * Refactors a k6 test for Vaadin compatibility with custom think time settings.
     *
     * @param inputFile       the input k6 test file
     * @param outputFile      the output refactored test file
     * @param thinkTimeConfig think time configuration for realistic user simulation
     * @throws MojoExecutionException if refactoring fails
     */
    public void refactorK6Test(Path inputFile, Path outputFile,
                               K6TestRefactorer.ThinkTimeConfig thinkTimeConfig) throws MojoExecutionException {
     ...
     * Runs a k6 load test.
     *
     * @param testFile     the k6 test file to run
     * @param virtualUsers number of virtual users
     * @param duration     test duration (e.g., "30s", "1m")
     * @param appIp        application IP address
     * @param appPort      application port
     * @throws MojoExecutionException if the test fails
     */
    public void runK6Test(Path testFile, int virtualUsers, String duration, String appIp, int appPort)
            throws MojoExecutionException {
     ...
     * @param appIp            application IP address
     * @param appPort          application port
     * @param useEmbeddedConfig if true, don't pass --vus and --duration (use script's embedded config)
     * @throws MojoExecutionException if the test fails
     */
    public void runK6Test(Path testFile, int virtualUsers, String duration, String appIp, int appPort,
                          boolean useEmbeddedConfig) throws MojoExecutionException {
        ...
        try {
            ...
            if (exitCode != 0) {
                throw new MojoExecutionException("k6 test failed with exit code: " + exitCode);
            }
    ...
    // Deprecated methods - kept for backwards compatibility but no longer needed

    /**
     * @deprecated Node.js is no longer required. This method always returns true.
     */
    @Deprecated
    ...
    /**
     * @deprecated npm dependencies are no longer required. This method is a no-op.
     */
    @Deprecated
    ...
    /**
     * @deprecated npm dependencies are no longer required. This method always returns true.
     */
    @Deprecated
