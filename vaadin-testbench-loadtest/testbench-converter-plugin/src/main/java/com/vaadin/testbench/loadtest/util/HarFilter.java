...
/**
 * Filters out requests to external domains from a HAR file.
 * This removes browser background traffic (Google services, telemetry, etc.)
 * that isn't part of the application under test.
 */
public class HarFilter {
    ...
    private static final Logger log = Logger.getLogger(HarFilter.class.getName());
     ...
     *
     * @param harFile the path to the HAR file
     ...
     *
     * @param inputFile  the input HAR file
     * @param outputFile the output HAR file (can be same as input)
     * @return the filter result with statistics
     * @throws IOException if reading or writing fails
     */
    public FilterResult filter(Path inputFile, Path outputFile) throws IOException {
        ...
        for (HarEntry entry : har.log().entries()) {
            if (entry.request() != null && entry.request().url() != null) {
                ...
                if (isExternalDomain(url)) {
                    String truncatedUrl = url.length() > 80 ? url.substring(0, 80) + "..." : url;
                    log.fine("  Filtered: " + truncatedUrl);
                    continue;
                }
                // Filter Vaadin session unload requests (sent when browser tab closes)
        ...
        HarFile filteredHar = new HarFile(
                new HarLog(har.log().version(), har.log().creator(), filteredEntries, har.log().pages())
        );
        ...
        log.info("Done! " + filteredCount + " of " + originalCount + " requests filtered.");
    ...
    /**
     * Check if a request is a Vaadin session unload (sent when the browser tab closes).
     * These contain {@code "UNLOAD":true} in the POST body and should not be replayed.
     */
    private boolean isUnloadRequest(HarEntry entry) {
        if (entry.request().postData() != null && entry.request().postData().text() != null) {
            return entry.request().postData().text().contains("\"UNLOAD\":true");
        }
        return false;
    }
     ...
     */
    public record FilterResult(int originalCount, int filteredCount, int remainingCount) {}
    ...
    public record HarRequest(
            ...
            List<HarQueryString> queryString,
    ...
    @JsonIgnoreProperties(ignoreUnknown = true)
    public record HarPostData(String mimeType, String text, List<HarParam> params) {}

    @JsonIgnoreProperties(ignoreUnknown = true)
    public record HarParam(String name, String value, String fileName, String contentType) {}

    @JsonIgnoreProperties(ignoreUnknown = true)
    public record HarContent(Long size, Long compression, String mimeType, String text, String encoding) {}

    @JsonIgnoreProperties(ignoreUnknown = true)
    public record HarCache(HarCacheEntry beforeRequest, HarCacheEntry afterRequest) {}

    @JsonIgnoreProperties(ignoreUnknown = true)
    public record HarCacheEntry(
            String expires,
            String lastAccess,
            String eTag,
            Integer hitCount
    ...
    public record HarTimings(
            ...
            Double connect,
            Double send,
            @JsonProperty("wait") Double waitTime,
            Double receive,
            Double ssl
    ) {}
}
