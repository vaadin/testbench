...
public class ProtocolHandshake {
    ...
    public Result createSession(HttpHandler client, Command command)
        ...
        Collection<Capabilities> desired = (Collection<Capabilities>) command
                .getParameters().get("capabilities");

        desired = desired == null || desired.isEmpty()
                ? singleton(new ImmutableCapabilities())
                : desired;

        try (NewSessionPayload payload = NewSessionPayload.create(desired)) {
            Either<SessionNotCreatedException, Result> result = createSession(
                    client, payload);

            if (result.isRight()) {
                Result toReturn = result.right();
                LOG.log(Level.FINE, "Detected upstream dialect: {0}",
                        toReturn.dialect);
                return toReturn;
            } else {
                throw result.left();
    ...
    public Either<SessionNotCreatedException, Result> createSession(
        ...
        int threshold = (int) Math.min(Runtime.getRuntime().freeMemory() / 10,
                Integer.MAX_VALUE);
    ...
    private Either<SessionNotCreatedException, Result> createSession(
            HttpHandler client, Supplier<InputStream> contentSupplier,
            long size) {
        ...
        // Setting the CONTENT_LENGTH will allow a http client implementation
        // not to read the data in
        // memory. Usually the payload is small and buffering it to memory is
        // okay, except for a new
        ...
        // Ignore the content type. It may not have been set. Strictly speaking
        ...
        return Stream.of(new W3CHandshakeResponse().getResponseFunction())
                ...
                .orElseGet(() -> Either.left(new SessionNotCreatedException(
    ...
    public static class Result {
        ...
        Result(Dialect dialect, String sessionId, Map<String, ?> capabilities) {
            ...
            if (capabilities.containsKey(PROXY)) {
                // noinspection unchecked
                ((Map<String, Object>) capabilities).put(PROXY,
                        massageProxy.apply(capabilities.get(PROXY)));
            }
        }
