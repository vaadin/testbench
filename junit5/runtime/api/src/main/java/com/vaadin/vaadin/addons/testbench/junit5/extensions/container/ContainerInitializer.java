 ...
 */
package com.vaadin.vaadin.addons.testbench.junit5.extensions.container;

import static com.vaadin.vaadin.addons.testbench.junit5.extensions.container.NetworkFunctions.freePort;
import static com.vaadin.vaadin.addons.testbench.junit5.extensions.container.NetworkFunctions.localeIP;

import java.lang.reflect.Method;

import org.junit.jupiter.api.extension.ExtensionContext;
import com.vaadin.dependencies.core.logger.HasLogger;
import com.vaadin.vaadin.addons.junit5.extensions.ExtensionFunctions;
...
public interface ContainerInitializer extends HasLogger {
  ...
  default String prepareIP(ExtensionContext context) {
    ...
    ExtensionFunctions.store().apply(context).put(NetworkFunctions.SERVER_IP, serverIP);
    logger().info(
  ...
  default int preparePort(ExtensionContext context) {
    ...
    ExtensionFunctions.store().apply(context).put(NetworkFunctions.SERVER_PORT, port);
    logger().info(
  ...
  default void cleanUpPort(ExtensionContext context) {
    ExtensionFunctions.store().apply(context).remove(NetworkFunctions.SERVER_PORT);
  }

  default void cleanUpIP(ExtensionContext context) {
    ExtensionFunctions.store().apply(context).remove(NetworkFunctions.SERVER_IP);
  }
